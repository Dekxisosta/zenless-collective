...
export default function Navbar() {
  ...
  // ← old inline ProfileAvatar const removed
  ...
  return (
    <nav className="sticky top-0 z-50">
      ...
      <div
        ...
        style={{ backdropFilter: "blur(16px)", backgroundColor: "var(--color-surface)", borderBottom: "1px solid var(--color-border)" }}
      >
        <div className="px-6 flex justify-end items-center gap-3">
          <Link to="#" className="transition hover:opacity-100 opacity-60 text-xs" style={{ color: "var(--color-text)" }}>
            Notifications
          </Link>
          ...
          {user ? (
            ...
            <>
              <Link to="/login" className="text-xs px-3 py-1 rounded-lg border transition hover:bg-white/10" style={{ borderColor: "var(--color-border)", color: "var(--color-text)" }}>
                Login
              </Link>
              <Link to="/signup" className="text-xs px-3 py-1 rounded-lg font-medium transition" style={{ backgroundColor: "var(--color-primary)", color: "white" }}>
                Sign Up
              </Link>
            </>
          )}
      ...
      {/* ── MAIN BAR ── */}
      <div className="py-3" style={{ backdropFilter: "blur(16px)", backgroundColor: "var(--color-surface)", borderBottom: "1px solid var(--color-border)" }}>
        <div className="px-6 flex items-center relative">
            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                <span
                  ...
                  style={{ width: isActive(link.to) ? "100%" : "0%", backgroundColor: "var(--color-primary)" }}
                />
              </Link>
            ))}
          </div>
          ...
          <Link to="/" className="absolute md:left-1/2 md:-translate-x-1/2 flex items-center">
            <img src={theme === "light" ? logo_light : logo_dark} className="h-16 w-auto" alt="logo" />
          </Link>
          ...
          <div className="md:hidden ml-auto flex items-center gap-3">
            <ProfileAvatar avatar={currentAvatar} />
            <button ref={hamburgerRef} className="flex flex-col gap-1 rounded-none" onClick={() => setOpen(!open)}>
              {[1, 2, 3].map((i) => (
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <Link to="#" onClick={() => setOpen(false)} className="text-sm opacity-70" style={{ color: "var(--color-text)" }}>Help</Link>
